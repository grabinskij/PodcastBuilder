...
def main():
    ...
    create_element(channel_element, 'itunes:author',
                   yaml_data.get('author', ''))
    create_element(channel_element, 'description',
                   yaml_data.get('description', ''))
    create_element(channel_element, 'itunes:image', None, {
                   'href': link_prefix + yaml_data.get('image', '')})
    ...
    if isinstance(category, dict):
        ...
        if main_category:
            category_element = create_element(
                channel_element, 'itunes:category', text=main_category)
            if subcategory:
                create_element(category_element,
    ...
    for item in yaml_data.get('item', []):
        ...
        create_element(item_element, 'itunes:author',
                       yaml_data.get('author', ''))
        create_element(item_element, 'description',
                       item.get('description', ''))
        create_element(item_element, 'itunes:duration',
                       item.get('duration', ''))
